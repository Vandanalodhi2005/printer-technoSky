...
const Printers = () => {
    ...
    // Fetch all products across all categories to manually filter ensuring data is found
    useEffect(() => {
        setAllProducts([]);
        setCurrPage(1);
        // Pass empty category to get everything first and then filter for "Printers" on frontend if precision fails
    ...
    const filteredAndSorted = useMemo(() => {
        // Filter for any product that has "Printer" in its category name OR title
        let filtered = allProducts.filter(p => {
            const catName = p.category?.name || p.category || '';
            const titleMatch = (p.title || p.name || '').toLowerCase().includes('printer');
            const catMatch = catName.toLowerCase().includes('printer');
            
            // Exclude anything that is explicitly Ink or Toner
            const isConsumable = catName.toLowerCase().includes('ink') || catName.toLowerCase().includes('toner');
            
            return (titleMatch || catMatch) && !isConsumable;
        });

        if (filtered.length === 0 && allProducts.length > 0) {
            // Fallback: If still nothing, just show things that aren't ink/toner
             filtered = allProducts.filter(p => {
                const catName = p.category?.name || p.category || '';
                return !catName.toLowerCase().includes('ink') && !catName.toLowerCase().includes('toner');
             });
        }
        ...
        <div className="category-view-elite">
            ...
            <header className="cat-hero-dark reveal">
                <div className="cat-hero-container">
                    <span className="cat-pretitle">Advanced Hardware</span>
                    <h1 className="cat-main-title text-white">Printers <br /><span className="text-[#B19777]">& Systems.</span></h1>
                    <p className="cat-meta-desc">
                        Engineering excellence for the modern enterprise. Explore our collection of 
                        high-performance laser, inkjet, and multifunction architectures.
                    </p>
                </div>
            ...
            <main className="cat-content-wrap">
                <div className="cat-utility-bar reveal">
                    <div className="utility-left">
                        <span className="results-indicator">AVAILABLE SYSTEMS: {filteredAndSorted.length}</span>
                    </div>
                    ...
                    {filteredAndSorted.length > 0 ? (
                        ...
                        !loading && (
                            <div className="col-span-full py-40 text-center">
                                ...
                                <p className="text-gray-400 text-[10px] mt-4 uppercase tracking-widest">Ensuring asset availability</p>
                            </div>
                        )
                    )}
                </div>
                ...
                {loading && (
                    <div className="cat-loading-status">
                         ...
                         <span>Querying Enterprise Hardware...</span>
                    </div>
                )}
